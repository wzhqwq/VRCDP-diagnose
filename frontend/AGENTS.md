...
- Timeline fetching keeps a LOD-like page cache for 50 ms and 100 ms metric windows. Render the full timeline and zoom placeholders with the coarse 100 ms data first; when a zoom range calls for 50 ms data, keep showing the 100 ms version while missing 50 ms pages load, then replace the rendered zoom data once those pages are cached. Full-timeline auto refresh should only request data after the cached last timestamp; manual refresh reloads the full timeline. Zoomed ranges should not auto-refresh, and range changes should load only missing pages at the range-selected window size.
...
- On this Windows workspace, use `node_modules\.bin\*.cmd` directly for frontend tooling instead of `npm run ...` when npm is not on PATH. If the `.cmd` wrappers cannot find `node`, prepend the bundled Node directory from `load_workspace_dependencies` to `PATH`, then run commands such as `.\node_modules\.bin\svelte-check.cmd --tsconfig .\tsconfig.app.json`, `.\node_modules\.bin\tsc.cmd -p .\tsconfig.node.json`, and `.\node_modules\.bin\vite.cmd build`.
